...
class MetricEnv(gym.Env):
    ...
    def init_obstacles_random_size(self):
        ...
        vels = [30 + 20 * (np.random.rand(2) - 0.5) for i in range(100)]
    ...
    def get_traversibility(self, axis_range):
        
        travers_sum = 0

        for (x, y) in product(axis_range, axis_range):
            travers_sum += traversibility(self.map_gt.grid_map, (x, y))

        return travers_sum / (len(axis_range)**2)
